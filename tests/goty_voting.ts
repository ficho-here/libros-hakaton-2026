...
describe("goty_voting", () => {
  ...
  const invalidOptionVoter = anchor.web3.Keypair.generate();
  ...
  let invalidOptionVotePda: anchor.web3.PublicKey;

  before(async () => {
    for (const wallet of [voter, invalidOptionVoter]) {
    ...
    [invalidOptionVotePda] = anchor.web3.PublicKey.findProgramAddressSync(
      [
        Buffer.from("vote"),
        pollPda.toBuffer(),
        invalidOptionVoter.publicKey.toBuffer(),
      ],
      program.programId,
    );
  });
  ...
  it("allows a wallet to vote", async () => {
    ...
    expect(vote.timestamp.toNumber()).to.be.greaterThan(0);
    expect(vote.bump).to.be.a("number");
    expect(
      poll.votes.map((count: anchor.BN) => count.toNumber()),
  ...
  it("rejects an invalid option", async () => {
    try {
      await program.methods
        ...
        .accounts({
          poll: pollPda,
          vote: invalidOptionVotePda,
          voter: invalidOptionVoter.publicKey,
          systemProgram: anchor.web3.SystemProgram.programId,
        })
        .signers([invalidOptionVoter])
        .rpc();

      expect.fail("Invalid option should have failed");
    } catch (error) {
      expect(String(error)).to.include("InvalidOption");
    }
  });
});
